/**
 * Usage Tracking Middleware for ProseCreator Publisher
 *
...
const SERVICE_NAME = 'prosecreator-publisher';
...
function detectOperation(req: Request): string {
  ...
  if (path.includes('/publishing')) {
    if (path.includes('/submit')) return 'publishing_submit';
    if (path.includes('/review')) return 'publishing_review';
    if (path.includes('/approve')) return 'publishing_approve';
    if (path.includes('/reject')) return 'publishing_reject';
    if (path.includes('/distribute')) return 'publishing_distribute';
    if (path.includes('/withdraw')) return 'publishing_withdraw';
    if (path.includes('/status')) return 'publishing_status';
    if (path.includes('/platforms')) return 'publishing_platforms';
    if (method === 'POST') return 'publishing_create';
    if (method === 'PUT' || method === 'PATCH') return 'publishing_update';
    if (method === 'DELETE') return 'publishing_delete';
    return 'publishing_read';
  }

  // ISBN operations
  if (path.includes('/isbn')) {
    if (path.includes('/request')) return 'isbn_request';
    if (path.includes('/assign')) return 'isbn_assign';
    if (path.includes('/validate')) return 'isbn_validate';
    if (path.includes('/lookup')) return 'isbn_lookup';
    if (path.includes('/barcode')) return 'isbn_barcode';
    if (path.includes('/reserve')) return 'isbn_reserve';
    if (method === 'POST') return 'isbn_create';
    return 'isbn_query';
  }

  // Format operations
  if (path.includes('/formats') || path.includes('/format')) {
    if (path.includes('/convert')) return 'format_convert';
    if (path.includes('/epub')) return 'format_epub';
    if (path.includes('/mobi')) return 'format_mobi';
    if (path.includes('/pdf')) return 'format_pdf';
    if (path.includes('/print')) return 'format_print';
    if (path.includes('/audio')) return 'format_audio';
    if (path.includes('/validate')) return 'format_validate';
    if (path.includes('/preview')) return 'format_preview';
    return 'format_operation';
  }

  // Cover operations
  if (path.includes('/covers') || path.includes('/cover')) {
    if (path.includes('/generate')) return 'cover_generate';
    if (path.includes('/upload')) return 'cover_upload';
    if (path.includes('/template')) return 'cover_template';
    if (path.includes('/design')) return 'cover_design';
    if (path.includes('/resize')) return 'cover_resize';
    if (path.includes('/validate')) return 'cover_validate';
    if (method === 'POST') return 'cover_create';
    if (method === 'PUT' || method === 'PATCH') return 'cover_update';
    if (method === 'DELETE') return 'cover_delete';
    return 'cover_read';
  }

  // Metadata operations
  if (path.includes('/metadata')) {
    if (path.includes('/onix')) return 'metadata_onix';
    if (path.includes('/marc')) return 'metadata_marc';
    if (path.includes('/export')) return 'metadata_export';
    if (path.includes('/import')) return 'metadata_import';
    if (path.includes('/validate')) return 'metadata_validate';
    if (path.includes('/enrich')) return 'metadata_enrich';
    if (path.includes('/keywords')) return 'metadata_keywords';
    if (path.includes('/categories')) return 'metadata_categories';
    if (method === 'POST') return 'metadata_create';
    if (method === 'PUT' || method === 'PATCH') return 'metadata_update';
